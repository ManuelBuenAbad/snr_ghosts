...
Tbg_408_avg = ap.bg_408_temp(0., 0., size=(4*pi), average=True) # total sky average
Tbg_408_antipodal = ap.bg_408_temp(180., 0., average=False) # antipodal point to galactic center
...
def check_source(source_input, custom_name='custom'):
    ...
    if not 'name' in source_input.keys():
        source_input['name'] = custom_name # update source 'source_input' dictionary to contain some name
    
    has_all_source_id = set(ap.source_id).issubset(set(source_input.keys()))
    if not has_all_source_id:
        raise KeyError("'source_input' does not contain all the keys in source_id={}. Please update.".format(ap.source_id))
    
    has_all_model_always = set(ap.pars_always).issubset(set(source_input.keys()))
    if not has_all_model_always:
        raise KeyError("'source_input' does not contain all the keys in pars_always={}. Please update.".format(ap.pars_always))
    
    is_there_model = ('model' in source_input.keys())
    if not is_there_model:
        raise KeyError("'source_input' does not have a model. Please update and make sure source_input['model'] is either 'eff' or 'thy'.")
    else:
        model = source_input['model']
        if model in ['eff', 'thy']:
            
            known = [] # list of lightcurve parameters that are known (i.e. present in kwargs)
            unknown = [] # list of lightcurve parameters that are to be deduced (i.e. are missing in kwargs)
            ...
            try:
                required = ap.pars_required[(model, unknown)] # the parameters required in source_input
            except:
                raise KeyError("Currently the code does not support having {} as an unknown parameter, only {}. Please update.".format(unknown, ap.pars_required.keys()))
            ...
            raise KeyError("source_input['model']={} is neither 'eff' nor 'thy', which are the only two options. Please update.".format(model))
...
def check_axion(axion_input):
    ...
    if not has_all_params:
        raise KeyError("'axion_input' dictionary does not contain all the keys: {}. Please update.".format(necessary_keys))
...
def check_data(data, deltaE_over_E=1.e-3, f_Delta=0.721, exper='SKA', total_observing_time=100., average=False, verbose=0):
    ...
    if not 'deltaE_over_E' in data.keys():
        data['deltaE_over_E'] = deltaE_over_E # update 'data' with default value for deltaE_over_E
    
    if not 'f_Delta' in data.keys():
        data['f_Delta'] = f_Delta # update 'data' with default value of f_Delta
    ...
    if not 'total_observing_time' in data.keys():
        data['total_observing_time'] = total_observing_time # update 'data' with default value for deltaE_over_E
    
    if not 'average' in data.keys():
        data['average'] = average # update 'data' with default value for deltaE_over_E
    
    necessary_keys = ['deltaE_over_E', 'f_Delta', 'exper', 'total_observing_time', 'average']
    has_all_params = all([key in data.keys() for key in necessary_keys])
    if not has_all_params:
        raise KeyError("'data' dictionary does not contain all the keys: {}. Please update.".format(necessary_keys))
...
def Snu_source(t, nu, source_input, output=None):
    ...
    local_source = {key:value for key, value in source_input.items()} # dictionary of parameters necessary to compute the luminosity of the source
    ...
    if not nu_is_scalar: # nu is an array
        ...
        for nn in nu:
            ...
            if model == 'thy':
                tau_factor = (nn/5.)**-2.1 # Weiler's frequency-dependent opacity correction factor
                local_source['tau_factor'] = tau_factor
            
            factor = ap.nu_factor(nn, nu_pivot, alpha) # frequency-dependent correction factor
            Lnu = factor*ap.L_source(t, output_pars=False, **local_source) # luminosity [erg * s^-1 * Hz^-1] w/ frequency-dependent factor
            Lum.append(Lnu)
        Lum = np.array(Lum)
        ...
        if model == 'thy':
            tau_factor = (nu/5.)**-2.1 # Weiler's frequency-dependent opacity correction factor
            local_source['tau_factor'] = tau_factor
        
        factor = ap.nu_factor(nu, nu_pivot, alpha) # frequency-dependent correction factor
        Lum = np.squeeze(factor*ap.L_source(t, output_pars=False, **local_source)) # luminosity [erg * s^-1 * Hz^-1] w/ frequency-dependent factor
        ...
        factor = ap.nu_factor(nu, nu_pivot, alpha) # frequency-dependent correction factor
        Lum = factor*ap.L_source(t, output_pars=False, **local_source) # luminosity [erg * s^-1 * Hz^-1] w/ frequency-dependent factor
...
def dSnu_echo(x, theta, # position of echo differential element
    ...
    t = tobs - 2.*x*ct._kpc_over_lightyear_ # [years] time at which echo differential element ought to be evaluated
    dist_from_gal_center = ap.r_to_gal(x, th=theta) # [kpc] distance of echo differential element to galactic center
    
    rho_at_x = rho(dist_from_gal_center) / ct._eV_over_GeV_/ct._cm_eV_**3  # [eV^4] dark matter density at location of echo differential element
    
    try:
        if t.ndim == 0:
            t = t[None]
        
        Snu_eval = Snu_fn(t[::-1]) # evaluating with inverted order in t (which from above we can see its in decreasing order). Will re-order later.
        Snu_eval = np.squeeze(Snu_eval)[::-1] # squeezing; then returning to original order; this is a feature of interpolation functions for the case when their arguments are in decreasing order instead of increasing.s
...
def Snu_echo(source_input, axion_input, data,
    ...
    theta_source = ap.theta_gal_ctr(l_source, b_source, output_radians=True) # [rad] angle of source to galactic center
    ...
    if 't_age' in source_input.keys(): # already among parameters
        ...
        if (source_input['model'] != 'eff'):
            raise KeyError("'t_age' is not among the keys of source_input, and yet the model in source_input['model']={} is not 'eff'. This should not have happened.")
    ...
    if (not recycle) or (type(output) != dict): # computing from scratch
        ...
        nu = pt.nu_from_ma(ma)
        tArr = np.logspace(log10(tmin), log10(t_age), Nt) # [years] array of times at which we compute the source's Snu
        SnuArr = Snu_source(tArr, nu, source_input, output=None) # [Jy]
    
    elif recycle and ('source_t-nu-Snu' in output.keys()): # recycling previously-computed sources
        tArr, nu, SnuArr = output['source_t-nu-Snu'] # Snu for source: t, nu, Snu
        nu_ma = pt.nu_from_ma(ma)
        diff = np.abs(nu_ma/nu - 1.)
        
        if (diff > 1.e-10):
            raise ValueError("Upon recycling the previously computed output['source_t-nu-Snu'], we found it to be inconsistent with the requested axion mass ma: the frequencies do not match: nu = {} GHz != {} GHz = ma/2. Please re-compute Snu_source with the correct frequency; or simply demand recycle=False.".format(nu, nu_ma))
        
    else:
        raise ValueError("'recycle_output' is not of the form (bool, dict). Please update.")
    ...
    x_offset = t_extra_old/(2.*ct._kpc_over_lightyear_) # the offset in the l.o.s. from the extra SNR age
    
    # the lower limit: xmin
    xmin += x_offset # adding offset
    
    # the upper limit: xmax
    xmax_tmp = (t_age - tmin)/(2.*ct._kpc_over_lightyear_) # the location of the wave front
    ...
    if use_quad:
        ...
        if lin_space:
            ...
            if xmax < xmax_default:
                ...
                t_lo = tage_extended - xmax*2.*ct._kpc_over_lightyear_ # lowest lightcurve time probed by l.o.s.
            
            t_hi = t_age - (xmin - x_offset)*(2.*ct._kpc_over_lightyear_) # highest lightcurve time probed by l.o.s.
            
            t_arr = np.logspace(log10(t_lo), log10(t_hi), Nint) # array of times, without the offset
            x_arr = ((t_age - t_arr)/(2.*ct._kpc_over_lightyear_))[::-1] # corresponding array of l.o.s. positions; inverted to go from lowest to largest, without the offset
...
def echo_an_sup(ma, ga, Lpk, dist, S0, t_age, gamma, tpk, deltaE_over_E=1.e-3):
    ...
    frac_tt_mask = 1.*np.heaviside(frac_tt-1., 1.) + frac_tt*np.heaviside(1.-frac_tt, 0.)*np.heaviside(frac_tt-frac_tpk, 0.) + frac_tpk*np.heaviside(frac_tpk-frac_tt, 1.)
...
def signal(source_input, axion_input, data,
    ...
    nu = pt.nu_from_ma(ma) # [GHz] frequency corresponding to half the axion mass
    ...
    if exper == 'SKA': # in case the range is frequency-dependent
        exper_mode = ap.SKA_exper_nu(nu) # could be either 'SKA low', 'SKA mid', or None, depending on frequency nu.
    ...
    signal_S_echo = (signal_Snu*ct._Jy_over_eV3_)*(delnu*ct._GHz_over_eV_) # [eV^4] irradiance (flux) over the bandwidth
    
    # compute the signal power
    signal_power = ap.P_signal(S=signal_S_echo, A=area, f_Delta=f_Delta) # [eV^2], assuming the default SKA efficiency of eta = 0.8
...
def noise(source_input, axion_input, data,
    ...
    exper = data['exper'] # experiment requested
    average = data['average'] # whether we average the noise brightness temperature over the angular size of the source
    ...
    Tbg_408_at_echo_loc = ap.bg_408_temp(l=l_echo, b=b_echo, size=Omega_echo, average=average, verbose=False) # [K]
    ...
    noise_power = ap.P_noise(T_noise=T_noise, delnu=delnu, tobs=obs_time) # [eV^2]
...
def ma_ga_bound(sn_val, ma_arr, sn_ref_arr, ga_ref):
    ...
    if len(ma_arr) != len(sn_ref_arr):
        raise ValueError("The arguments 'ma_arr' and 'sn_ref_arr' must be of the same length, since the latter are the signal-to-noise ratios computed for the axion masses given by the former.")
