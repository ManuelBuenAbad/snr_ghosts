...
# lightcurve params
pars_lightcurve = {model:pars_early[model]+['gamma']+pars_late for model in ['eff', 'thy']}
...
# all params:
source_input = {key:source_id+pars_always+value+pars_late for key, value in pars_early.items()}
...
def irrad(distance, Lum):
    ...
    Area = 4.*pi*(distance*ct._kpc_over_cm_)**2. # [cm^2] sphere covering the source
...
def L_free(t, model='eff', **kwargs):
    ...
    # Checking kwargs:
    pars_inter = set(pars_early[model]).intersection(kwargs.keys()) # subset of kwargs that is in pars_early[model]
    if pars_inter != set(pars_early[model]):
        raise KeyError("The kwargs passed do not contain all the arguments for the model requested. For model={} these are: {}".format(model, pars_early[model]))
...
def sandwich_logeqn(Lpk, tpk, L0, t0, gamma, tt):
    ...
    eqn = log10(Lpk/L0) + (1.5)*(1. - tpk_yr/tt)*log10(np.e) + (gamma-1.5)*log10(tt) + 1.5*log10(tpk_yr) - gamma*log10(t0)
...
def L_source(t, model='eff', output_pars=False, **kwargs):
    ...
    if not 'gamma' in kwargs.keys():
        raise KeyError("'gamma' is not in kwargs. Please pass 'gamma' as an argument.")
    gamma = kwargs['gamma']
    
    known = {} # list of lightcurve parameters that are known (i.e. present in kwargs)
    to_deduce = [] # list of lightcurve parameters that are to be deduced (i.e. are missing in kwargs)
    ...
    required = pars_required[(model, to_deduce)] # the parameters required in kwargs
    ...
    if model == 'eff':
        ...
        if to_deduce == 'L_today': # based on peak info, deduce L_today
            ...
            L_trans = L_free(t_trans, model=model, **known) # computing transition luminosity
            ...
            L_trans = L_adiab(t_trans, **adiab_kwargs) # computing transition luminosity
            def LogLeff_fn(LogLpk): return log10(L_eff(t_trans, L_peak=10**LogLpk, **eff_kwargs)) - log10(L_trans)
            ...
            def fn(Ltt): return sandwich_logeqn(L_peak, t_peak, L_today, t_age, gamma, 10.**Ltt) # function to minimize
            try:
            ...
            def Lf_fn(tt): return L_free(tt, model=model, **known) # simplified free expansion function
            
            L_trans = Lf_fn(t_trans) # L_trans
            adiab_kwargs = {'L_ref':L_trans, 't_ref':t_trans, 'gamma':gamma}
            t_age = tage_compute(L_peak, t_peak, t_trans, L_today, gamma) # age
            known.update({'t_age':t_age})
        
        else:
            raise ValueError("to_deduce={}, known={} is currently not supported.".format(to_deduce, known))
        ...
        if 'L_today' in to_deduce:
            ...
            for par, val in known.items():
                if par in thy_pars:
                    thy_kwargs[par] = val
            L_trans = L_adiab(t_trans, **adiab_kwargs) # computing transition luminosity
            def LogLthy_fn(LogLnorm): return log10(L_thy(t_trans, L_norm=10**LogLnorm, **thy_kwargs)) - log10(L_trans)
...
def SKA_specs(nu, exper_mode):
    ...
    if exper_mode == None:
        ...
        window = np.heaviside(nu - ct._nu_min_ska_low_, 1.) * np.heaviside(ct._nu_max_ska_low_ - nu, 1.)
        Tr = 40.
    elif exper_mode == 'SKA mid':
        area = ct._area_ska_mid_
        window = np.heaviside(nu - ct._nu_min_ska_mid_, 0.) * np.heaviside(ct._nu_max_ska_mid_ - nu, 1.)
        Tr = 20.
...
def bg_408_temp(l, b, size=None, average=False, verbose=False):
    ...
    if average and size != None:
        ...
        new_pos_pix = hp.query_disc(nside=512, vec=vec, radius=ct.solid_angle_to_angle(size))
    else:
        new_pos_pix = pos_pix
...
def P_noise(T_noise, delnu, tobs):
    ...
    res = 2. * T_noise * ct._K_over_eV_ * np.sqrt(delnu * ct._GHz_over_eV_/(tobs * ct._hour_eV_))
    return res
